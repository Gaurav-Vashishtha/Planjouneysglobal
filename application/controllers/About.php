...
class About extends CI_Controller {
    ...
    $best_agency_image_main = $about->best_agency_image_main ?? ''; 
    if (!empty($_FILES['best_agency_image_main']['name'])) {
        log_message('debug', 'Uploading file for best_agency_image_main');
        $best_agency_image_main = $this->_upload_file(
            'best_agency_image_main',
            FCPATH . 'uploads/home/best_agency_main',  
            'jpg|jpeg|png|webp',
            $about->best_agency_image_main ?? null  
        );

    } else {
        log_message('debug', 'No file uploaded for best_agency_image_main');
    }
        ...
        $payload = [
            ...
            'best_agency_image_main'        =>  $best_agency_image_main,
            ...
            'bottom_icon' => $post['bottom_icon'] ?? '',
            'best_agency_link' => $post['best_agency_link'] ?? ''
        ];

    //  log_message('debug', print_r($payload, true));
    //  die(); 
...
private function _upload_file($field, $path, $types, $old = null)
{
    if (!is_dir($path)) {
        if (mkdir($path, 0755, true)) {
            log_message('debug', "Directory created: $path");
        } else {
            log_message('error', "Failed to create directory: $path");
        }
    } else {
        log_message('debug', "Directory exists: $path");
    }
    $config = [
        'upload_path'   => $path,
        'allowed_types' => $types,
    ...
    if (!empty($_FILES[$field]['name'])) {
        if ($old && file_exists($path . $old)) {
            unlink($path . $old);
            log_message('debug', "Old file deleted: $path$old");
        }
        if ($this->upload->do_upload($field)) {
            return $this->upload->data('file_name');
        } else {
            log_message('error', "File upload failed for field '$field': " . $this->upload->display_errors());
            return $old; 
        }
    }
    return $old;
